		...
		foreach ($files as $file => $info) {
			...
			try{
				...
				if(!empty($options['url'])){
					if($uploaded_image_extension=='jpg' || $uploaded_image_extension=='JPG' || $uploaded_image_extension=='jpeg' || $uploaded_image_extension=='JPEG'){
						$new_name = image_fix_orientation($options['url']."/".$new_name,$options['directory'],$new_name);
					}
				}
			}
			...
			if(isset($options['thumbs']) and is_array($options['thumbs'])){
				...
				if($ext == 'jpg' or $ext == 'png' or $ext == 'jpeg' or $ext == 'JPG' or $ext == 'JPEG' or $ext == 'PNG'){
					...
					$ImageCrop->uploadThumb(array_merge($thumb_config,array("destination_path"=>$options['directory']."/60","crop"=>true ,"size"=>60,"ratio"=>false)));
					$ImageCrop->uploadThumb(array_merge($thumb_config,array("destination_path"=>$options['directory']."/160","crop"=>true ,"size"=>160,"ratio"=>false)));
					$ImageCrop->uploadThumb(array_merge($thumb_config,array("destination_path"=>$options['directory']."/thumb","crop"=>true ,"size"=>300,"ratio"=>false)));
